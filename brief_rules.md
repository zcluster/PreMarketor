...
6. 正文必须图文并茂并包含以下模块，顺序固定为：关键资产图与关键词云图（同一个置顶 `module-grid`）→ 重大新闻汇总 → 美股夜盘行情总结 → 盘前结论 → 重点个股推荐 → 板块轮动看板 → 逻辑链 → 风险雷达 → 数据来源与时间点。
   ...
   - 重点个股推荐：必须保留 stock-picks 模块，可按证据质量输出 0-5 张 stock-card；不得为凑数推荐。每张卡必须包含 stock-name、stock-ticker、stock-badge，以及三段：推荐理由、触发因素、风险点。A股/港股早报优先覆盖 A股、港股、恒生科技相关个股；推荐理由必须与当日新闻、资金、财报、政策或板块逻辑相关。每个第 8c 条重大异动主题必须独立完成业务映射与选股决策：有已核验业务关联且通过筛选的 A/H 候选时，优先安排对应 stock-card；未纳入的主题必须在现有正文中点名候选及具体排除理由，无可核验候选时明确说明检索结果。禁止用一张泛科技或其他主题的“美股映射”卡代替全部主题的检查。
...
8c. 【重大美股异动到 A/H 候选的映射】08:30 首轮与 09:15 增量复核均检查会实质影响 A/H 的隔夜美股板块和异常个股，流程为“重大异动 → 业务映射核验 → 候选或无候选结论 → 风险与价格反馈”。
   - 从第 8a 条已取得的行业/概念排行及本轮重大新闻中选择真正具有跨市场影响的主题，不扩大榜单抓取范围。保留来源、板块/公司、涨跌幅、行情时间和触发因素；同一经济主题可合并研究，但不能遗漏会改变判断的重大负面异动。
   - 每个重大主题优先定向核验 1-3 个 A股或港股候选的公司名称、代码、实际业务/收入/资产敞口、关联机制、原始证据链接及披露日期。区分直接受益、间接产业链与纯情绪关联；没有可靠候选时说明检索限制，不编造股票。
   - 对进入正文或股票卡的主题保存简洁结构化决策：候选、业务证据、最新可得价格反馈、开盘验证、失效条件、decision 与具体理由。中英文结论与数值一致；未进入推荐的重大主题在新闻、结论、轮动或风险模块说明原因。此记录是轻量核验，不要求全榜单穷举、固定候选数或旧筛选证据契约。
   - 09:15 只复核首轮截止后的新增、遗漏或时效失效证据；只有实质改变判断时才合并更新。重大负面公告、明确利空催化或明显负面价格反馈不得包装成多头推荐。
8d. 【推荐候选质量与反证检查】适用于全部 A/H 股票卡，不设置市值、盈利或估值倍数的机械入选/排除阈值。
   - 推荐必须有可追溯的公告、政策、财报、业务资料或明确行业映射，并写推荐理由、触发因素和风险点。原始公告应同时提取利好机制与成本、稀释、债务、审批、执行及不确定性；分拆、融资、并购、回购等不能仅凭事件名称判为利好。
   - 提交前读取每只拟推荐股的最新可得价格、涨跌幅、行情时间和交易阶段；开市后必须使用当日价格反馈。公告后下跌、低开、弱于基准或其他明显负面反馈与正面逻辑冲突时，必须披露并转为观察/回避，除非已有可验证的反转确认。
...
3a. 重大异动映射与推荐安全：执行第 8c/8d 条，对正文实际使用的重大美股主题及 A/H 候选核对来源、业务关联、最新可得价格反馈、触发和失效条件；中英文、首页与归档决策一致。重大负面公告或明显负面价格反馈若被包装成多头推荐必须判定失败。此检查不要求全榜单穷举、固定候选数、市值/盈利/PE 阈值、固定长度交易日行情或固定区间收益。
...
3b. 个股推荐校验：逐一检查实际 stock-card 具有名称、ticker、推荐理由、触发因素、风险点、可追溯事实来源及最新可得价格反馈；重大负面公告或明显负面价格反馈未被正确降级时阻塞。市值、盈利、PE、近期收益若有可作风险参考，但任何缺失或旧机械阈值均不是发布门禁，不得因此自动删除全部卡片。
...
- 对实际推荐和会改变大盘/板块/风险判断的候选完成可追溯核验即可：保存核心来源、催化、业务关联、最新可得价格反馈、主要风险、触发与失效条件。个别可选估值或历史行情字段缺失时披露限制并继续，不得把“数据未取得”等同于不合格。
